...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, Iam Shubham' is in the banner component
	expect(screen.getByText(/Hi, Iam Shubham/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('Shubham-Resume.pdf');
	// expect(downloadLink.href).toEqual('/files/Shubham-Resume.pdf');
